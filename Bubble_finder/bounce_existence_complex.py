...
#   so Newton is performed in real arithmetic without "projecting away" imaginary parts.
#
...
#       - Add ONE constraint at a (pin_site) fixing the local amplitude ρ(pin_site) = ρ_pin
...
#   - The pinned amplitude is defined consistently with the solver definition:
#         u = Re( φ_rot * φbar_rot )
#         ρ = sqrt( smooth_pos(u) + ρ_eps )
#     using the same smooth_pos as the solver (and its derivative) for a stable Jacobian row.
#   - We pin on a site with j>=2 (avoid tiny r); default pin_site=(4,0).
#   - The constraint is built in terms of the *physical* u (includes 1/r^2), not the unscaled s,
#     to avoid the "λ blows up at small r" pathology.
#
# API
#   Primary entry point:
...
#
# NOTES
#   - This module does not modify EOM/BC/twist: it only builds an augmented Newton system.
#   - If you want to "polish" the unpinned candidate, call solver.solve(x_fields_star) in the notebook.
...
class ExistenceScanResult:
    ...
    zero_crossings: List[Tuple[int, int]]  # indices in points list
...
def _rho_and_grads_at_site(
    ...
    Uses the solver's smooth_pos and ρ_eps for consistency with the EOM/Jacobian.
    """
    j0, i0 = site
    ...
    *,
    ...
    for it in range(1, int(max_iter) + 1):
        ...
        if not np.isfinite(nF):
            return NewtonResultAug(x=x, success=False, iterations=it, residual_norm=nF, history=hist, last_step=last_alpha,
                                   info={"error": "Non-finite residual."})

        if nF < float(tol):
            return NewtonResultAug(x=x, success=True, iterations=it, residual_norm=nF, history=hist, last_step=last_alpha, info=info)
        ...
        if not ok:
            return NewtonResultAug(x=x, success=False, iterations=it, residual_norm=nF, history=hist, last_step=alpha,
                                   info={"error": "Line search failed."})

    return NewtonResultAug(x=x, success=False, iterations=int(max_iter), residual_norm=float(hist[-1]), history=hist, last_step=last_alpha,
                           info={"error": "max_iter reached"})
...
def _normalize_anchor_which(anchor_which: str) -> str:
    """
    Accept a few legacy aliases and map to RI-equation labels.
    """
...
def make_pinned_system(
    ...
    *,
    ...
    """
    Build (residual_aug, jacobian_aug, k_eq) for the pinned system.

    Unknowns: x_aug = [x_fields, lambda_pin]  (length N_fields + 1)

    Equations:
      - Field equations: F(x_fields)=0 except at k_eq where we do F_k + lambda_pin = 0
      - Pin constraint: g(x_fields)=ρ(pin_site) - ρ_pin = 0

    anchor_site defaults to pin_site.
    anchor_which selects which *equation row* to free:
      one of ('y_re','y_im','yb_re','yb_im') (legacy aliases 'y','ybar' are accepted).
    """
    ...
    def jacobian_aug(x_aug: np.ndarray) -> sp.csc_matrix:
        ...
        # Column for lambda: only one 1 at row k_eq
...
def scan_pinned_branch(
    ...
    *,
    ...
    """
    Track the pinned branch across rho_pin values.
    """
    _require_complex_saddle(solver)
    ...
    # detect sign changes of lambda among successful points
...
def refine_lambda_zero_by_bisect(
    ...
    *,
    ...
    """
    Bisection in rho_pin to find lambda_pin ~ 0.

    Returns: (rho_star, x_aug_star, lam_star)
    """
    _require_complex_saddle(solver)
    ...
    for it in range(int(max_iter)):
        ...
        if not nr_m.success:
            # If mid fails: shrink interval towards endpoint with smaller |lambda|
...
def verify_unpinned_solution(
    ...
    *,
    ...
    """
    Check whether x_fields_seed solves the original (unpinned) equations by evaluating ||F||.
    """
    _require_complex_saddle(solver)
...
def existence_driver(
    ...
    *,
    ...
    One-stop convenience:
      1) scan pinned branch
      2) if a sign change occurs, bisect to lambda≈0
      3) report whether an unpinned solution is found

    Returns a dict with:
      - scan (ExistenceScanResult)
      - exists (bool)
      - refined (optional dict with rho_star, lam_star, x_aug_star, unpin diagnostics)
    """
    ...
    # pick the first bracket crossing
